	...
	public void select(final Context context, final BeerDatabaseHelper helper, final int id, final String name) {
		switch (action) {
			...
			case Selectable.REMOVE_ACTION:
				...
				Cursor c1 = context.getContentResolver().query(BeerCellarProvider.CONTENT_URI,
				...
				if (c1.moveToFirst()) {
					...
					if (noBottles == 1) {
						int rows = context.getContentResolver().delete(BeerCellarProvider.CONTENT_URI, "_id = ?", new String[] { String.valueOf(c1.getInt(0)) });
